...
def generate_answer(req: GenerateRequest):
    ...
    # build results HTML (not generated by LLM)
    jobs_html = ""
    for r in results:
        title = r.get("title", "")
        url = r.get("url", "#")
...
1. Write ONLY one short introduction paragraph.
2. The introduction must be MAXIMUM 4 lines.
3. Do NOT describe jobs.
4. Do NOT create numbered lists.
5. Do NOT list job titles.
6. Do NOT suggest other websites.
7. After the introduction, ask 3 follow-up questions.
8. Use ONLY <p> tags.

Output format:

<p>Short introduction (1-4 lines) about the search results on Hozpitality.</p>
