...
def rebuild_fts():
    """
    Rebuild the V6 PostgreSQL full-text search vector.

    IMPORTANT:
    This must use only columns that currently exist in the
    production master_search_mastersearchindex table.

    V6 searchable sources:
        - title
        - category_text
        - user_name
        - location_text
    ...
    ai_search_text contains the relationship-aware canonical
    search document generated by this script.
    """
    ...
    try:
        with conn.cursor() as cur:
            print("Starting V6 FTS rebuild...", flush=True)
            ...
            cur.execute("""
                ...
                SET search_vector_v6 =
                    ...
                    || setweight(
                          to_tsvector(
                              'simple',
                              unaccent(coalesce(category_text, ''))
                          ),
                          'A'
                      )
                    ...
                    || setweight(
                          to_tsvector(
                              'simple',
                              unaccent(coalesce(location_text, ''))
                          ),
                          'B'
                      )
                    ...
                    || setweight(
                          to_tsvector(
                              'simple',
                              unaccent(coalesce(ai_search_text, ''))
                          ),
                          'B'
                      )
            ...
            print(
                f"FTS rows rebuilt: {cur.rowcount}",
                flush=True,
            )
