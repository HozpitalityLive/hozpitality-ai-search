        ...
        *,
        ...
        while tool_iterations < self.config.max_tool_iterations:
            ...
            if response.is_tool_call():
                ...
                # Tool-call content is internal model output (for example JSON
                # or SQL generated by Qwen). Never expose it in the chat UI.
                # The actual tool result is rendered below, followed by the
                # final natural-language response from the model.
                yield UiComponent(  # type: ignore
                    rich_component=StatusBarUpdateComponent(
                        status="working",
                        message="Finding the best results...",
                        detail=f"Running {len(response.tool_calls or [])} tool(s)",
                    )
                )
